...
export default function CometAiPageClient({
  stutantVersionLabel,
  dailiaVersionLabel,
}: {
  stutantVersionLabel: string;
  dailiaVersionLabel: string;
}) {
  const { t } = useLang();
  ...
  const dailiaFeatures = [
    ...
    { Icon: IconSearch, ko: "멤버십 등급별 모델 선택", en: "Model choice by membership tier" },
  ];
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div className="rounded-2xl p-7 border" style={{ borderColor: CYAN(0.25), background: CYAN(0.05) }}>
            ...
            <p className="text-white/45 text-sm leading-relaxed">
              {t(`Stutant ${stutantVersionLabel} · 정답을 넘어 학습 과정을 잇는 AI`, `Stutant ${stutantVersionLabel} · Beyond answers — connecting the learning process`)}
            </p>
          </div>
          <div className="rounded-2xl p-7 border" style={{ borderColor: VIOLET(0.25), background: VIOLET(0.05) }}>
            ...
            <p className="text-white/45 text-sm leading-relaxed">
              {t(`DAILIA ${dailiaVersionLabel} · 생각과 작업을 정리하는 일상 AI`, `DAILIA ${dailiaVersionLabel} · Organizing thoughts and tasks for daily life`)}
            </p>
          </div>
        </div>
      </section>
