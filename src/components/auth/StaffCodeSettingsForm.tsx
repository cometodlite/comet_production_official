"use client";

import { useActionState } from "react";
import { changeStaffCode, type AuthFormState } from "@/app/actions/auth";

const initialState: AuthFormState = {};

export default function StaffCodeSettingsForm() {
  const [state, action, pending] = useActionState(changeStaffCode, initialState);

  return (
    <form action={action} className="space-y-5">
      {state.errors?.form && (
        <div className="rounded-lg border border-red-500/25 bg-red-500/10 px-4 py-3 text-sm text-red-200">
          {state.errors.form[0]}
        </div>
      )}
      {state.success && (
        <div className="rounded-lg border border-emerald-400/25 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-100">
          {state.success}
        </div>
      )}

      <Field
        id="currentStaffCode"
        label="현재 사원 코드"
        name="currentStaffCode"
        type="password"
        autoComplete="off"
        placeholder="현재 사용 중인 코드"
        errors={state.errors?.currentStaffCode}
      />
      <Field
        id="newStaffCode"
        label="새 사원 코드"
        name="newStaffCode"
        type="password"
        autoComplete="off"
        placeholder="6자 이상"
        errors={state.errors?.newStaffCode}
      />
      <Field
        id="confirmStaffCode"
        label="새 사원 코드 확인"
        name="confirmStaffCode"
        type="password"
        autoComplete="off"
        placeholder="새 코드를 다시 입력"
        errors={state.errors?.confirmStaffCode}
      />

      <button
        type="submit"
        disabled={pending}
        className="w-full rounded-lg bg-white px-5 py-3 text-sm font-bold text-black transition hover:bg-amber-200 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {pending ? "변경 중..." : "사원 코드 변경"}
      </button>
    </form>
  );
}

function Field({
  id,
  label,
  errors,
  type = "text",
  ...props
}: {
  id: string;
  label: string;
  errors?: string[];
} & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      <label htmlFor={id} className="mb-2 block text-sm font-medium text-white/85">
        {label}
      </label>
      <input
        id={id}
        type={type}
        required
        className="w-full rounded-lg border border-white/10 bg-white/[0.06] px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/[0.28] focus:border-indigo-400/70 focus:bg-white/[0.09]"
        {...props}
      />
      {errors && (
        <div className="mt-2 space-y-1">
          {errors.map((error) => (
            <p key={error} className="text-xs text-red-300">
              {error}
            </p>
          ))}
        </div>
      )}
    </div>
  );
}
